...
AS
BEGIN
	SELECT DISTINCT CuocKham.ThoiGianKham, BenhNhan.CMND, BenhNhan.Ho, BenhNhan.Ten
	FROM BenhNhan INNER JOIN
		(CuocKham INNER JOIN 
				...
				ON 
					KetQuaXetNghiem.MaKhamBenh = CoXetNghiem.MaKhamBenh
					AND KetQuaXetNghiem.MaXN = CoXetNghiem.MaXN 
					AND KetQuaXetNghiem.GhiChu = 'BatThuong') 
		ON 
			CuocKham.MaKhamBenh = KetQuaXetNghiem.MaKhamBenh 
			AND (CuocKham.MaBacSi = @ma_bac_si OR CoXetNghiem.MaBSXN = @ma_bac_si))					
